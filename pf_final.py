...
class robot:
    ...
    # according to
    #           self.bearing_noise
    def sense(self, noise=bearing_noise): #do not change the name of this function
        Z = []
        for i in range(len(landmarks)):
            delta_y = landmarks[i][0] - self.y
            delta_x = landmarks[i][1] - self.x
            global_theta = atan2(delta_y, delta_x)
            global_theta = global_theta - self.orientation
            global_theta += random.gauss(0.0, noise)
            global_theta %= 2.0 * pi
            Z.append(global_theta)
